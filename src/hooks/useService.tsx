...
function useService() {
  ...
  async function getServices(search: string) {
    try {
      ...
      const result = await axios.get(
        `https://home-service-server.onrender.com/v1/admin/services/?search=${search}`
      );
  ...
  async function getServiceById(serviceId: number) {
    try {
      setLoading(true);
      const result = await axios.get(
        `https://home-service-server.onrender.com/v1/admin/services/${serviceId}`
      );
  ...
  async function deleteService(serviceId: number) {
    try {
      setLoading(true);
      const deleteImage = services.find(
        (service: { id: number }) => service.id === serviceId
      );
      // console.log("deleteImage --->", deleteImage)
      await secretKey.storage.from("testing").remove([deleteImage.pic_service]);
  ...
  async function createService(serviceData: any) {
    try {
      ...
      await axios.post(
        "https://home-service-server.onrender.com/v1/admin/services/",
        requestData
      );
      setLoading(false);
      setFormData(null);
  ...
  async function createSubService(subserviceData: any) {
    try {
      ...
      setLoading(true);
      await axios.post(
        "https://home-service-server.onrender.com/v1/admin/services/subservices/",
        requestData
      );
      setLoading(false);
      navigate("/services");
  ...
  async function downloadFile() {
    ...
    try {
      ...
      const { data, error } = await secretKey.storage
        .from("testing")
        .download(newService.pic_service);
