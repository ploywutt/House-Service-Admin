...
function Topbar_search(prop: TopbarType) {
	...
	}, [searchCategory, searchService, searchPromotion]);
			...
			<form className='flex items-center gap-6'>
				...
				</div>
				{
					prop.title === "Promotion Code" ?
						<Button className='h-11 py-2.5 px-6 gap-2' onClick={() => { navigate(prop.path) }}>เพิ่ม{` Promotion Code`} <span><img src={add} alt="add" /></span></Button> :
