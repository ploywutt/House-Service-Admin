...
function ImageInput() {
  ...
  async function uploadFile(file: any) {
    try {
      const { data, error } = await secretKey.storage
        .from("testing")
        .upload(`HomeService/${file.name}`, file);
  ...
  async function deleteFile(file: any) {
    await secretKey.storage.from("testing").remove([file]);
  }
  ...
  useEffect(() => {
    if (
      ...
      if (
        newService?.pic_service.includes("HomeService") &&
        formData?.image.name.includes(newService?.pic_service)
      ) {
  ...
  return (
    <div>
            ...
            <>
              ...
              <span className="text-gray-500 text-sm text-center font-normal leading-tight">
                <input
                  ...
                  value={fileList}
                />
                <span className="text-blue-600 text-sm font-normal leading-tight">
                  อัพโหลดรูปภาพ
                </span>{" "}
                หรือ ลากและวางที่นี่ <br />
      ...
      <div className="flex justify-between">
        <div className="text-gray-500 text-xs font-normal leading-none mt-2">
          ขนาดภาพที่แนะนำ: 1440 x 225 PX
        </div>
        {fileList && (
          <button
            className="border border-violet-500 px-2 py-1 rounded-md border-none text-blue-600 text-base font-semibold underline leading-normal"
            onClick={() => {
              setFileList(null);
            }}
          >
